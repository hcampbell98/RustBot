...
public class AcceptInvite : ModuleBase<SocketCommandContext>
{
    ...
    public async Task Accept()
    {
        ...
        if (TeamUtils.pendingInvites.ContainsKey(Context.User.Id)) 
        {
            ...
            await ReplyAsync($"<@!{Context.Message.MentionedUsers.First().Id}>", false, Utilities.GetEmbedMessage("Team Invite", $"{Context.User.Username}'s Team", $"You have joined the team.", Context.User));
            TeamUtils.pendingInvites.Remove(Context.User.Id);
            
        }
        else
        {
            await ReplyAsync("", false, Utilities.GetEmbedMessage("Team Invite", $"{Context.User.Username}'s Team", $"You have no pending invites.", Context.User));
        }
    }
}